...
export function ExhibitorsPage(){
  ...
  const openDialog=(item:(typeof exhibitors)[number])=>{previousFocus.current=document.activeElement as HTMLElement;setActive(item)};
      ...
      <div className="container">
        ...
        {list.length?(
          <div className="cards-grid exhibitors-grid">
            {list.map(e=>(
              <article className="card exhibitor-card" key={e.id}>
                <figure className="exhibitor-media">
                  {e.image?.src ? <img src={e.image.src} alt={e.image.alt} loading="lazy"/> : <div className="exhibitor-media-fallback" aria-hidden="true"/>}
                  <figcaption className="exhibitor-image-overlay" aria-hidden="true">
                    <span className="exhibitor-monogram">{e.name.slice(0,2).toUpperCase()}</span>
                    <span className="exhibitor-sector-overlay">{e.sector}</span>
                  </figcaption>
                </figure>
      ...
      <div className="dialog-backdrop" role="presentation" onMouseDown={closeDialog}>
        ...
        >
          ...
          <div className="exhibitor-dialog-media">
            {active.image?.src ? <img src={active.image.src} alt={active.image.alt}/> : <span aria-hidden="true">{active.name.slice(0,2).toUpperCase()}</span>}
            {!active.image?.src && <span aria-hidden="true">{active.name.slice(0,2).toUpperCase()}</span>}
          </div>
